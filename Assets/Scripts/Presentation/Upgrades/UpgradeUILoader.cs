            ...
            try
            {
                ...
                if (_loadHandle.Status == AsyncOperationStatus.Succeeded)
                {
                    ...
                    if (_upgradeUIInstance != null)
                    {
                        ...
                        // FIX: Parent to DynamicPanels if found, otherwise keep as is (but do NOT use DontDestroyOnLoad)
                        // This ensures it shares the correct Canvas and Raycaster settings.
                        var dynamicPanels = GameObject.Find(Santa.Core.Config.GameConstants.Hierarchy.DynamicPanels);
                        if (dynamicPanels != null)
                        {
                            instantiatedObject.transform.SetParent(dynamicPanels.transform, false);
                        }
                        else
                        {
                            // Fallback: Try to find UIManager and parent to it
                            var uiManager = Object.FindFirstObjectByType<UIManager>();
                            if (uiManager != null)
                            {
                                instantiatedObject.transform.SetParent(uiManager.transform, false);
                            }
                        }

                        // Ensure it is last sibling to be on top
                        instantiatedObject.transform.SetAsLastSibling();

                        // FIX: Ensure it renders on top of CombatUI (which has order 5000)
                        var canvas = instantiatedObject.GetComponent<Canvas>();
                        if (canvas == null) canvas = instantiatedObject.AddComponent<Canvas>();

                        canvas.overrideSorting = true;
                        canvas.sortingOrder = 5500; // Above CombatUI (5000) but below PauseMenu (6000)

                        // Ensure Raycaster exists for input
                        if (instantiatedObject.GetComponent<UnityEngine.UI.GraphicRaycaster>() == null)
                        {
                            instantiatedObject.AddComponent<UnityEngine.UI.GraphicRaycaster>();
                        }

                        // Object.DontDestroyOnLoad(instantiatedObject); // <-- REMOVED: This was causing the issue
